...
## Выбор стратегии маскировки

| Режим | Назначение | Требования к сертификату | Рекомендация |
| :--- | :--- | :--- | :--- |
| **Обфускация** (Salamander) | Превращает трафик в "кашу" (шум). Нужна, если провайдер блокирует конкретные протоколы. | Можно использовать **самоподписной** (бесплатный) сертификат, так как он скрыт внутри обфускации. | Оптимально, если нет домена и блокировки не слишком агрессивные. |
| **Мимикрия** (Стандартный QUIC) | Притворяется обычным HTTPS-сайтом. Нужна, если провайдер блокирует любой неизвестный UDP-трафик. | Требуется **валидный сертификат** (Let's Encrypt). Самоподписной в этом режиме приведет к блокировке. | **Рекомендуется использовать домен**, так как это самый надежный способ обхода продвинутых DPI. |
